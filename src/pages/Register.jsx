...
const registerSchema = z.object({
  ...
  username: z
    ...
    .max(20, "The username must be a maximum of 20 characters."),
});

export const Register = () => {
  const [error, setError] = useState(null);
  const { loginGoogle, registerUser } = useContext(AuthContext);
      ...
      <div className="max-w-md w-full bg-[#1E293B] rounded-xl shadow-lg p-8">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <button
            onClick={loginGoogle}
            type="button"
            className="mt-2 bg-white text-gray-700 border border-gray-300 rounded-lg px-4 py-3.5 font-semibold cursor-pointer text-sm transition duration-200 flex items-center justify-center gap-2.5 w-full"
          >
            <img
